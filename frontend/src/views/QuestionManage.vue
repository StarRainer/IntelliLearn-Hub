        ...
        <div class="step-content">
          ...
          <div v-if="aiGenerateStep === 0" class="params-step">
            <el-form :model="aiGenerateForm" :rules="aiGenerateRules" ref="aiFormRef" label-width="120px">
			  ...
			  <el-radio-group v-model="aiGenerateForm.types">
			    <el-radio label="CHOICE">选择题</el-radio>
			    <el-radio label="JUDGE">判断题</el-radio>
			    <el-radio label="TEXT">简答题</el-radio>
			  </el-radio-group>
			</el-form-item>
...
const aiGenerateRules = {
  ...
  types: [{ required: true, message: '请选择至少一种题目类型', trigger: 'change' }],
...
const getAllCategories = () => {
  ...
  // 1. 如果没有选择类型，或者类型为空字符串 -> 返回所有分类（保留了你原来的逻辑）
  if (!aiGenerateForm.types) {
...
const getSelectedTypesText = () => {
  ...
  // 直接通过 Map 获取，取不到就显示原值
  return typeMap[aiGenerateForm.types] || aiGenerateForm.types
}
